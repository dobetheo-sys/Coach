...
export function buildSwimrunSessions(kit: SessionKit): V1Session[] {
  ...
  if (slot === "durLong") {
    ...
    if (phase === "dev" && kit.weekNum % 2 === 1 && !kit.noVo2 && !medHold && !beginner
    ...
    } else if ((phase === "spec" || phase === "peak") && kit.weekNum % 2 === 0) {
      ...
      //
      // Correctif (10/09/2026, `feuvertswimruncss.md`) — cette condition portait AUSSI
      // `|| kit.r.weeks <= 14`, qui forçait cette branche INCONDITIONNELLEMENT (donc éliminait
      // le seuil css) sur tout plan de 14 semaines ou moins — la quasi-totalité des plans
      // swimrun réels (experience/sprint/series tiennent tous sous ce seuil par construction).
      // Mesuré (règle 15) : au-dessus de 14 semaines l'alternance fonctionnait déjà comme
      // annoncée ci-dessus (≈50/50) ; le membre `weeks <= 14` n'a jamais fait partie de
      // l'alternance qu'il prétendait servir — retiré, `BUGS_OUVERTS.md` entrée « S15-css ».
