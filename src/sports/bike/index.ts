...
export function buildBikeSessions(kit: SessionKit): V1Session[] {
  const { a, fmt, slot, phase, lvl, finisher, noVo2, inj, S2, P, W, C, B } = kit;
  const clm = fmt === "clm", climb = a.terrain === "montagne" || a.terrain === "vallonne";
  // Audit 08/08/2026 — le vélo était le seul sport sans aucune substitution de séance liée à
  // une blessure : R6.1/R6.2 (reasoningEngine) réduisent déjà le volume et avertissent si la
  // discipline principale est en conflit (genou/dos → `bk` interdit, ANX-GEN), mais rien
  // n'ADAPTAIT la séance elle-même — même geste que run.ts (B2/R6.1 : « genou → seuil
  // contrôlé, pas de pics de force »). `inj.lumbar` existait déjà dans `InjuryInfo`
  // (constraintMatrix.ts) et n'était lu NULLE PART dans le dépôt jusqu'ici.
  const kneeSpares = inj.list.includes("genou");
  const backSpares = inj.lumbar; // dos — la position agressive + le gros braquet chargent le bas du dos
  if (slot === "dur1") {
    ...
    // Genou fragile : le VO2max (puissance maximale = force maximale sur la pédale à chaque
    // coup) perd ses pics de force, pas le stimulus aérobie — cadence haute plutôt qu'effort
    // maximal, comme le seuil contrôlé de run.ts.
    else if (kneeSpares && (phase === "spec" || phase === "peak" || phase === "dev") && !noVo2) S2.push({ d: "bk", name: "Sweetspot contrôlé (genou épargné)", note: "Genou fragile : on garde le stimulus aérobie fort mais sans les pics de force du sprint — cadence haute (95-100 rpm), effort soutenu plutôt que maximal.", det: "", steps: [W(15, "montée progressive, cadence libre"), B(P(3, 4), P(10, 15), "bk.ss", "4min souple", " cadence 95-100 rpm"), C(10, "décrassage")] });
    ...
    // Genou ET dos fragiles sont contraindiqués par le MÊME geste (gros braquet + cadence
    // basse = force maximale par coup de pédale sur l'articulation, et position agressive
    // soutenue = charge lombaire) — un seul remplacement, cadence normale + position relâchée,
    // le libellé nomme la zone réellement en cause.
    else if (kneeSpares || backSpares) S2.push({ d: "bk", name: "Seuil position détendue" + (backSpares ? " (dos épargné)" : " (genou épargné)"), note: (backSpares ? "Dos fragile : la position agressive et le gros braquet chargent le bas du dos" : "Genou fragile : le gros braquet à cadence basse charge l'articulation à chaque coup de pédale") + " — remplacé par un effort soutenu à cadence NORMALE (85-95 rpm), position relâchée, mêmes minutes d'effort.", det: "", steps: [W(15, "progressif, position relâchée"), B(P(3, 4), P(8, 15), "bk.thr", "3min souple, redresse-toi"), C(10, "décrassage")] });
