...
function s5IdentiteR202(plan: V1Plan): string[] {
  // O-105 (fiche 44 T9) — LE GARDE LIT L'ARGMIN PUBLIÉ, jamais un min() brut recalculé : deux
  // computations de « le maillon qui borne » libres de diverger sont la forme R11.1 que ce
  // dépôt refuse partout ailleurs, et le min() brut mesurait une grandeur que le produit
  // n'affiche pas (la garde d'observation du moteur écarte les plafonds que le livré réfute).
  const r = (plan as { _r202?: { argmin?: string; plafonds?: { id?: string; livre?: number }[] } })._r202;
  ...
  if (!argmin || typeof argmin.livre !== "number")
    return ["argmin publié (" + String(r.argmin) + ") absent des plafonds du record"];
  return Math.abs(argmin.livre - pic) > 0.1
    ? ["plafond publié (" + String(r.argmin) + ") = " + argmin.livre.toFixed(2) + " h ≠ pic livré " + pic.toFixed(2) + " h"]
    : [];
}
