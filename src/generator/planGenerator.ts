 ...
 *
 * La répartition de l'axe DISCIPLINE est PROPORTIONNELLE : chaque séance de la discipline se
 * voit demander le même facteur `f = ref / total`. Elle suppose donc que **chaque séance peut
 * payer** — or une séance ÉPINGLÉE (palier B-17 : la distance EST le stimulus, et le plan l'a
 * ANNONCÉE) n'en paie AUCUNE part, et les planchers de bloc bornent les autres. Une passe
 * unique sous-livre alors exactement la part des séances qui ne peuvent pas payer, et la garde
 * ne converge que par répétition : mesuré, une seconde passe change **22 plans sur 1 074, tous
 * en `tri/Full`** (`tri/Full/reprise/inter` S25 « Nage seuil (+dist) » 1 225 → 1 100 m, S28
 * 575 → 225 m — la « Nage continue » épinglée de la même semaine pèse 69 et 84 min et ne bouge
 * pas d'un mètre). C'est la famille « protégé par le chemin, pas par la borne », dans l'autre
 * sens : la répartition suppose un payeur là où il y a un protégé.
 *
 * On converge donc SUR PLACE, en un nombre de tours BORNÉ et connu — même forme que la
 * saturation du maillon `structurel` —, et on sort dès qu'un tour ne bouge plus. Quand plus
 * personne ne peut payer (la séance épinglée dépasse à elle seule la référence), le tour ne
 * bouge pas, la boucle sort, et le résidu est STRUCTUREL : ce n'est pas un défaut d'arbitrage,
 * c'est une séance annoncée que la garde n'a pas le droit de raboter — le banc `lotPhysio` la
 * met d'ailleurs HORS CHAMP et la COMPTE (`epinglees`).
 *
 * 8 : DEUX tours de marge au-dessus de la convergence MESURÉE. Balayage du corpus complet à
 * bornes croissantes, plans changés d'un cran au suivant : 1→2 **22** · 2→3 15 · 3→4 14 ·
 * 4→5 13 · 5→6 **3** · 6→8 **0**. Le corpus converge donc exactement à **6** — la marge est
 * posée parce qu'un profil futur pourrait demander un tour de plus, et qu'une borne atteinte
 * en silence rendrait la garde non idempotente sans que rien ne le dise. Les deux tours de
 * marge sont mesurés INERTES (N=6 et N=8 rendent le corpus au bit près).
 */
const T56_TOURS_MAX = 8;
...
function enforceRecupSousCharges(plan: V1Plan, render?: (s: V1Session) => void): void {
  ...
  for (let i = 0; i < wl.length; i++) {
    ...
    const dAv = av ? discMin(av) : { sw: 0, bk: 0, rn: 0 }, dAp = ap ? discMin(ap) : { sw: 0, bk: 0, rn: 0 };
