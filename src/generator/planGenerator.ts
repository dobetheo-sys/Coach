...
import {
  ...
  RN1_INTERVAL_SEMAINES, RN1_ENTRETIEN_REPS, RN1_ENTRETIEN_REPS_MIN, RN1_ENTRETIEN_DUR_MIN, readInjuries,
...
export const _o113: { wk: number; avant: number; apres: number; semaine: number }[] = [];
...
export function reconcileDeclaredVolume(
   ...
   */
  function abaisserLongueVersPart(): void {
    for (const wk of plan.weeks) {
 ...
 */
function swimCapDebutantM(baseM: number, cssSecPer100m: number | undefined): number {
 ...
 * le signale.
 */
const REFERENCES = (av: V1Week | null, ap: V1Week | null): (V1Week | null)[] => [av];
...
function enforceRecupSousCharges(plan: V1Plan, render?: (s: V1Session) => void): void {
  ...
  const estEpinglee = (s: V1Session) => (s.steps || []).some((st) => st.bnd?.pinned);
  const discMin = (w: V1Week, sansEpinglees?: boolean) => {
    const t: Record<string, number> = { sw: 0, bk: 0, rn: 0 };
    for (const d of w.days) for (const s of d.sessions) {
      if (s.d === "rs" || s.race) continue;
  ...
  for (let i = 0; i < wl.length; i++) {
    ...
    const typesVus = new Set<string>(), discVues = new Set<string>();
...
export function generatePlan(profile: AthleteProfile, opts?: { noLoadFactor?: boolean }): { plan: V1Plan; reasoned: ReasonedPlan } {
  ...
  const tailleActuelle = (b: V1Step): number => (b.distanceM != null ? b.distanceM : (b.durationMin ?? 0));
  ...
  function blockBounds(b: V1Step, s: BoundedSession): { floor: number; cap: number } {
    ...
    const sansBorne = (floor: number) => ({ floor, cap: _sondeSansBorne ? Math.max(floor, tailleActuelle(b)) : 9999 });
  ...
  // Blessure d'APPUI déclarée (« course », ou toute zone d'impact — même lecture que le module
  ...
  if (a.sport === "tri" && a.intent === "competition" && String(a.level) !== "debutant" && String(a.history) !== "reprise" && !injRn1.list.includes("course") && !injRn1.impact) {
    ...
    const vo2VeloMin = (wd: GenDay[]) => wd.reduce((t, d) => t + d.sessions.filter((s) => s.d === "bk").reduce((u, s) => u + (s.steps || []).filter((b) => /\.vo2$/.test(String(b.zone || ""))).reduce((v, b) => v + (b.reps || 1) * (b.durationMin || 0), 0), 0), 0);
    const nageSeuilM = (wd: GenDay[]) => wd.reduce((t, d) => t + d.sessions.filter((s) => s.d === "sw").reduce((u, s) => u + (s.steps || []).filter((b) => b.zone === "sw.css").reduce((v, b) => v + (b.reps || 1) * (b.distanceM || 0), 0), 0), 0);
    ...
    const receveurFacile = (wd: GenDay[], cible: V1Session) => {
      const candidats: { s: V1Session; corps: V1Step }[] = [];
    ...
    eligiblesRn.forEach((wk, idx) => {
      ...
      for (;;) {
        ...
        cible.steps = [
          ...
          { role: "body", reps, durationMin: RN1_ENTRETIEN_DUR_MIN, zone, intensity: intOf(zone) as unknown as string, recoveryText: "2min trot", recoveryMin: 2, suffix: "", prefix: "" } as V1Step,
  ...
  // RC1 — REPOS COMPLET GARANTI EN MULTISPORT + BIKE (feu vert du fondateur, 09-10/09/2026,
  // `feuvert2et3.md` puis `decisionrc1bikeetscope4.md`). Domaine dérivé (règle O-85) pour la
  // partie multisport : `disciplines.length > 1`, jamais une liste ["tri","duathlon"] —
  // `swimrun` y entre aussi mais n'a rien à convertir (son propre schéma livre déjà un vrai
  // OFF sur 100 % des semaines mesurées, voir `RC1_LABEL_RECUP`). Post-construction (même
  // emplacement que FV1, mêmes raisons) : la couverture des disciplines et le plancher de
  // fréquence tournent déjà sur ce jour au moment de sa construction, une conversion en amont
  // serait rouverte par eux.
  //
  // `bike` (pur) EST TRAITÉ ICI DEPUIS LE 10/09/2026 : mesuré à 12,1 % de vrai OFF, quasiment
  // le même défaut que tri/duathlon (9,7-10,2 % avant correctif) — la prémisse « bike déjà
  // correct » était fausse (trouvaille publiée le 09/09, confirmée et actée par le fondateur
  // le lendemain). Ajouté par disjonction explicite plutôt que par un domaine dérivé plus
  // large (`disciplines.length >= 1`) : `run`/`swim`/`trail` restent MESURÉS corrects
  // (77,7 % / 31,7 % / 100 %) et n'ont reçu aucune autorisation — un domaine dérivé les
  // aurait inclus sans mesure ni décision, target exactement ce que règle 7 interdit.
  if (sportModule(a.sport as string).disciplines.length > 1 || a.sport === "bike") {
    for (const wk of plan.weeks) {
  ...
  // BQ1 — RETIRÉ, NON LIVRÉ (voir `BUGS_OUVERTS.md`, entrée « BQ1 » et le rapport envoyé au
  // fondateur, 10/09/2026) : le mécanisme identifié dans la décision (« doublage de nage »)
  // ne représentait que 32 des 1535 semaines à ≥3 créneaux mesurées — la source RÉELLE et
  // très majoritaire est `facile2` (`sports/tri/index.ts`, R13.3/C3), la nage QUALITÉ que le
  // code nomme lui-même « la SEULE nage de la semaine » pour un athlète qui ne double pas.
  // Un plafond à 2 qui l'atteindrait retirerait au triathlète sa seule séance de nage
  // construite sur la majorité des semaines de dev/spec/peak — l'exact dommage que R13.3
  // existe pour empêcher. Non implémenté sans nouvelle décision.
  ...
  // (drapeau médical, blessure, âge) — hiérarchie du manifeste, santé d'abord.
  {
    ...
    if (L.declared > 0) {
      ...
      let structBrut = Math.min(_sondeCapH > 0 ? _sondeCapH : Infinity, L.c20 > 0 ? L.c20 : Infinity);
      {
        ...
        const minutesDe = (ds: GenDay[]) => ds.reduce((t, d) => t + d.sessions.reduce((u, s) => u + (s.min || 0), 0), 0);
        let picCycle: GenDay[] | null = null;
