 ...
 *
 * ⚠ RÉSERVE MESURÉE (20/08/2026) — **LA PART DE 20 % EN NAGE NE TIENT QU'À PARTIR DE ~13 H DE
 * VOLUME HEBDOMADAIRE**, et c'est la troisième fois que le volume et la répartition se révèlent
 * inséparables :
 *
 *     à 11,2 h   20 % = 2,2 h  →  3 séances de 45 min (la fréquence tombe sous ce que la
 *                                 technique demande) ou 4 de 33 min (le palier B-17 en veut ~41)
 *     à 13,0 h   20 % = 2,6 h  →  4 séances de 39 min ✓
 *
 * En dessous, la cible force un arbitrage entre la FRÉQUENCE et la LONGUEUR, et pour un nageur
 * limité par la technique aucun des deux ne se sacrifie bien. L'écart affiché à 11,2 h est donc
 * en partie une conséquence du VOLUME, pas seulement du routage — le dire au lieu de le corriger.
 *
 ...
 */
export const ALLOC_CIBLE_PALIERS: Record<string, { maxH: number; cible: Record<string, number> }[] | undefined> = {
...
export function allocCibleDe(sport: string, declaredH: number | undefined): Record<string, number> | undefined {
