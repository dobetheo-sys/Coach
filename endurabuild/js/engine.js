...
const C30_PART_SEMAINE_PIC = 0.70;
...
function predictTri(kit            )       {
  ...
  const capte = (lo        , hi        , k                         ) => { bornes[k] = [lo, hi]; };
...
function predictDuathlon(kit            )       {
  ...
  const capte = (t        , k             ) => { bornes[k] = [t * (bnd ? bnd[0] : 1) * 0.97, t * (bnd ? bnd[1] : 1) * 1.03]; };
...
function reconcileDeclaredVolume(
   ...
   */
  function raiseLongRunToSpecificity()       {
...
function predictRace(
  ...
  const render = (args            )                                                                                                 => {
    ...
    const fmtRange = (lo        , hi        ) => { note(lo, hi); return fmtT(lo) + "–" + fmtT(hi); };
...
function hrvBaseline(journal                         , date        )              {
  const vide              = { n: 0, suffisant: false, moyenneMs: null, basseMs: null, hauteMs: null,
  ...
  const dansFenetre = (j        ) => propres.filter((x) => joursEntre(x.date, date) <= j);
