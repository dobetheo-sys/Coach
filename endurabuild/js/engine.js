 ...
 */
const RACE_EVE_CAP_MIN = rule("C27", "la veille d'une course : ≤45 min faciles — jamais la plus longue séance de la semaine", 45);
...
function easyShareFloor(weeklyMin        )         {
...
function buildTriSessions(kit            )              {
  ...
  } else if (slot === "facile2") S2.push({ d: "sw", recovery: true, name: swShort.name + " courte", note: swShort.note, det: "", steps: swShort.steps, ...( { plainBody: true }          ) });
  ...
  // entraîneur vérifie en premier en relisant une semaine.
  {
    ...
    const shrinkTo = (sx           , capMin        )       => {
      ...
      for (let g = 0; g < 5 && totalOf(sx) > capMin + 0.5; g++) {
        ...
        for (const st of sx.steps || []) {
          if (st.role !== "body") continue;
          // Seuls les blocs qui portent du DÉNIVELÉ sont intouchables ici : la charge verticale
          // (T1/T2/T2b) a ses propres passes, et un plafond de libellé n'a pas à déplacer un axe
          // de charge sans que sa progression soit revérifiée. Un bloc `flat` n'en porte aucun —
          // l'exclure aussi laissait 9 « Footing récup » de 71 min et 18 sorties longues de trail
          // dépassées par une autre séance, pour une prudence sans objet.
          if (st.gradient === "up" || st.gradient === "down" || st.gradient === "rolling") continue;
          if ((st.reps || 1) > 1) {
            const next = Math.max(1, Math.round((st.reps || 1) * f));
            if (next < (st.reps || 1)) { st.reps = next; touched = true; }
          } else if (st.durationMin) {
...
function generatePlan(profile                , opts                             )                                           {
  ...
  const keepsMainDiscipline = (wd2          , victim        )          => {
    ...
    const covers = (d        ) => d.sessions.some((s) => s.d === mainD || (s.d === "br" && (s.steps || []).some((b) => b.leg === (mainD === "rn" ? "run" : mainD === "bk" ? "bike" : "swim"))));
  ...
  for (const rc of races) {
    ...
    if (wk) {
        ...
        rd.sessions = [{
          ...
          det: rc.prio === "A"
            ...
            : rc.prio === "C"
              ...
              : "Course de préparation : mini-affûtage fait, tu peux appuyer. Départ contrôlé, finis fort. — 💡 Objectif : valider allures et stratégie avant l'objectif A.",
          min: prevMin,
          ...
          steps: [{ role: "body", durationMin: prevMin, _min: prevMin }],
      ...
      for (let k = 1; k <= prep; k++) {
        const d = dayAt(-k);
        if (!d) continue;
        const capMin = k === 1 ? RACE_EVE_CAP_MIN : Math.round(RACE_EVE_CAP_MIN * 1.5);
        d.charge = "facile";
        for (const sx of d.sessions) {
          if (sx.d === "rs" || sx.race || !sx.steps) continue;
          if ((sx.min || 0) <= capMin) continue;
          const mainD = sportModule(a.sport          ).mainDiscipline;
          sx.steps = [{ role: "body", durationMin: capMin, zone: mainD + (mainD === "sw" ? ".easy" : mainD === "bk" ? ".z2" : ".easy") }          ];
          sx.long = false;
          sx.brick = false;
          sx.name = k === 1 ? "Déverrouillage (veille de course)" : "Endurance allégée (avant course)";
          sx.note = k === 1
            ? "Veille de course : on réveille les jambes, on ne les fatigue pas. Court, souple, quelques accélérations de 20 s si tu en ressens le besoin — puis on range les chaussures."
...
function buildPlanV2(sport        , answers            )                                {
  ...
  const res = generateAudited(toProfile(sport, vr.answers                         ));
  const plan = res.plan                                 ;
