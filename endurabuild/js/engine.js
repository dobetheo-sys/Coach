...
function generatePlan(profile                , opts                             )                                           {
  ...
  reconcileDeclaredVolume(plan, r.warnings, (s) => renderSess(s, refs, r.hz, r.baseRefs), { swimFloors: guard(a.sport          , "swimSessionFloors"), beginner: r.beginner, medHold: r.medHold, keepTaperSwim: guard(a.sport          , "swimRacePrepFrequency") && !r.dbl && !r.medHold, mainDiscipline: sportModule(a.sport          ).mainDiscipline, disciplines: sportModule(a.sport          ).disciplines });
...
function generateAudited(profile                , auditOpts                     )              {
  ...
  reconcileDeclaredVolume(best.plan, warnings, (s) => renderSess(s, refs, reasoned.hz, reasoned.baseRefs), { swimFloors: guard(reasoned.profile.sport          , "swimSessionFloors"), beginner: reasoned.beginner, medHold: reasoned.medHold, keepTaperSwim: guard(reasoned.profile.sport          , "swimRacePrepFrequency") && !reasoned.dbl && !reasoned.medHold, mainDiscipline: sportModule(reasoned.profile.sport          ).mainDiscipline, disciplines: sportModule(reasoned.profile.sport          ).disciplines });
...
const BUCKET_BY_LEVEL                         = { debutant: "debutant", inter: "inter", avance: "avance" };
const BUCKET_BY_HISTORY                         = { reprise: "debutant", confirme: "inter", ancien: "avance" };
...
function projectForm(input                 )                   {
  ...
  for (const k of ["ftp", "thrPace", "css", "vam"]         ) {
    const prior = g[k] * sat;
    let brut = prior;
...
function buildProjection(
  ...
  const vide = { applicable: false, horizonWeeks: p.horizonWeeks, adherence: p.adherence, gainPct: p.gainPct,
    ...
    refs, items: []                    , decisions: p.decisions };
