...
function buildPlan(a){
  ...
  function sess(slot,phase,prog){
    ...
    if(sp==="run"){
      ...
      // FIX régression : 3 créneaux nage = 3 contenus réellement distincts
      // (principal distance / technique-vitesse / récup courte), pas le même texte sous 3 noms.
      ...
      const swTech=beginner
        ?{name:"Nage éducatifs",det:struct({ech:"100m souple",corps:P(6,10)+"×50m éducatifs (rattrapé, poings fermés, battements planche), 1 point technique par longueur",rec:"20-30s, le temps de respirer",rc:"100m dos souple",note:"Zéro chrono ici : uniquement le geste. Alterne les éducatifs, ne les enchaîne pas en force."})}
        :{name:"Nage vitesse",det:struct({ech:"200m + 4×25m accélérations progressives",corps:P(6,10)+"×50m @ "+sz.speed+" départ toutes les 60-75s",rec:"30-40s (récup large)",rc:"150m souple",note:"Fréquence et vitesse contrôlées : la technique ne doit pas se dégrader sur les derniers 50m."})};
      ...
      else if(slot==="dur2"){ if(dbl)S2.push({d:"sw",name:swTech.name,det:swTech.det}); S2.push({d:"bk",name:"Force basse cadence",det:struct({ech:"15min + montée en intensité",corps:P(4,6)+"×5min @ "+bz.frc+" à 50-60 rpm",rec:"3min souple",rc:"10min moulinage",note:"Gros braquet, cadence basse : musculaire, pas cardio. Sans forcer sur les genoux."})}); }
      else if(slot==="durLong"){
